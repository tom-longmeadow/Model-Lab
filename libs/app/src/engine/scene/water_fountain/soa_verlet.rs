...
use crate::{
    engine::{input::InputState, scene::{Scene, water_fountain::{scene_config::WaterFountainSceneConfig, verlet_soa_stream_lifecycle::WaterFountainSoaStreamLifecycle}, }},
    graphics_context::{
        GraphicsContext,
...
impl<V: Vector> WaterFountainParticleSoaVerletScene<V> {
    pub fn new() -> Self {
        Self { 
    ...
    V: From<(f64, f64)>, 
{
    fn build_passes(&mut self, renderer: &mut GraphicsContext) {
        ...
        let hz = WaterFountainSceneConfig::hz();
        let env = WaterFountainSceneConfig::environment();
        let config = WaterFountainSceneConfig::config();
        let sim = Simulation::new(
            hz,
            <VerletParticleSoaVecStorage<V> as CpuStorage>::new(config.max_particles),
            VerletSoaGravitySolver::new( config.max_particles, V::Scalar::ONE, true ),
            WaterFountainSoaStreamLifecycle::<V>::new(config),
            env,
        );    
