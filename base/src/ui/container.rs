...
impl WidgetContainer {
    ...
    pub fn gap(&self) -> Option<f32> { self.gap }
    pub fn set_gap(&mut self, gap: f32) { self.gap = Some(gap); }
    pub fn clear_gap(&mut self) { self.gap = None; }
    pub fn resolved_gap(&self, fallback: f32) -> f32 { self.gap.unwrap_or(fallback) }
