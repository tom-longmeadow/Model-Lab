...
pub struct NewtonianLinearDrag {
    pub coeff: f64,
}
impl NewtonianLinearDrag {
...
/// Requires explicit stored velocity (Newtonian / Leapfrog / VelocityVerlet).
pub struct NewtonianDamping {
    pub factor: f64,
}
impl NewtonianDamping {
...
pub struct NewtonianBallConstraint<const N: usize> {
    pub center:      [f64; N],
    pub radius:      f64,
    pub restitution: f64,
}
impl<const N: usize> NewtonianBallConstraint<N> {
...
/// `restitution` ∈ `[0.0, 1.0]` — `1.0` = perfectly elastic, `0.0` = fully inelastic.
pub struct NewtonianDimConstraint {
    pub min: f64,
    pub max: f64,
    pub restitution: f64,
}
impl NewtonianDimConstraint {
...
mod tests {
    ...
    fn linear_drag_reduces_acc_proportional_to_vel() {
        let drag = NewtonianLinearDrag::new(2.0);
        let mut acc = 0.0;
    ...
    fn linear_drag_zero_vel_no_effect() {
        let drag = NewtonianLinearDrag::new(5.0);
        let mut acc = 10.0;
    ...
    fn linear_drag_negative_vel_increases_acc() {
        let drag = NewtonianLinearDrag::new(1.0);
        let mut acc = 0.0;
    ...
    fn damping_scales_velocity() {
        let damp = NewtonianDamping::new(0.9);
        let mut vel = 10.0;
        damp.apply(&mut vel);
    ...
    fn damping_factor_zero_stops_particle() {
        let damp = NewtonianDamping::new(0.0);
        let mut vel = 100.0;
        damp.apply(&mut vel);
        assert!((vel).abs() < EPS);
    }
    ...
    fn damping_factor_one_no_change() {
        let damp = NewtonianDamping::new(1.0);
        let mut vel = 7.5;
        damp.apply(&mut vel);
    ...
    fn dim_constraint_below_min_clamps_and_reflects() {
        let c = NewtonianDimConstraint::new(0.0, 10.0, 1.0);
        let mut pos = -1.0;
        let mut vel = -5.0;
    ...
    fn dim_constraint_inelastic_loses_speed() {
        let c = NewtonianDimConstraint::new(0.0, 10.0, 0.5);
        let mut pos = -1.0;
        let mut vel = -4.0;
        c.apply(&mut pos, &mut vel);
        assert!((pos - 0.0).abs() < EPS);
    ...
    fn ball_constraint_inside_no_effect() {
        let c = NewtonianBallConstraint::new([0.0, 0.0], 5.0, 1.0);
        let mut pos = [1.0, 0.0];
        let mut vel = [1.0, 0.0];
        c.apply(&mut pos, &mut vel);
    ...
    fn ball_constraint_outside_clamps_to_surface() {
        let c = NewtonianBallConstraint::new([0.0, 0.0], 1.0, 1.0);
    ...
    fn ball_constraint_inelastic_reduces_speed() {
        let c = NewtonianBallConstraint::new([0.0, 0.0], 1.0, 0.0);
        let mut pos = [2.0, 0.0];
